...
class PredictionService:
    ...
    def _real_predict(self, df: pd.DataFrame, horizon: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Prédiction LSTM: {len(historical_predictions)} historiques + {len(future_predictions)} futures"
            )
    ...
    def _fallback_predict_OLD_DISABLED(self, df: pd.DataFrame, horizon: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ ANCIEN FALLBACK (DÉSACTIVÉ): {len(historical_predictions)} historiques + {len(future_predictions)} futures"
            )
