...
class DataService:
    ...
    def _load_combined_data(self, ticker: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"✅ Données combinées: {len(df_hist_filtered)} historiques + {len(df_realtime)} temps réel = {len(df_combined)} total")
            return df_combined
    ...
    def _load_features_data(self, file_path: Path, ticker: str) -> pd.DataFrame:
        ...
        feature_cols = [
            ...
            'EMA_RATIO', 'ATR_NORMALIZED', 'VOLUME_RATIO', 'PRICE_POSITION',
            'ROC_10', 'RETURNS', 'RETURNS_MA_5', 'RETURNS_MA_10', 'RETURNS_MA_20', 'RETURNS_MA_50',
            'MOMENTUM_5', 'MOMENTUM_10', 'MOMENTUM_20', 'VOLUME_PRICE_TREND',
            'PRICE_VELOCITY', 'PRICE_POSITION_20'
        ]
