...
class SentimentAnalyzer:
    ...
    def add_sentiment(self, ticker: str, sentiment: float, confidence: float = 1.0):
        ...
        self.sentiment_data[ticker] = [
            (ts, sent, conf) for ts, sent, conf in self.sentiment_data[ticker]
            if ts > cutoff
        ]
    ...
    def add_volume(self, ticker: str, volume: float):
        ...
        cutoff = timestamp - timedelta(hours=24)
        self.volume_data[ticker] = [
            (ts, v) for ts, v in self.volume_data[ticker]
            if ts > cutoff
        ]
    ...
    def get_sentiment(self, ticker: str, window_minutes: int = None) -> float:
        ...
        recent_sentiments = [
            sent for ts, sent, conf in self.sentiment_data[ticker]
            if ts > cutoff
        ]
        ...
        recent_data = [
            (sent, conf) for ts, sent, conf in self.sentiment_data[ticker]
            if ts > cutoff
        ]
    ...
    def get_volatility(self, ticker: str, window_minutes: int = 20) -> float:
        ...
        recent_prices = [
            price for ts, price in self.price_data[ticker]
            if ts > cutoff
        ]
    ...
    def get_volume_ratio(self, ticker: str, current_volume: float, window_minutes: int = 20) -> float:
        ...
        cutoff = datetime.now() - timedelta(minutes=window_minutes)
        recent_volumes = [
            volume for ts, volume in self.volume_data[ticker]
            if ts > cutoff
        ]
    ...
    def get_adaptive_sentiment(self, 
                              ticker: str, 
                              sentiment: float, 
                              volatility: float, 
        ...
        # Ajustements basés sur le volume
