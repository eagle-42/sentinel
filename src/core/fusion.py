...
class AdaptiveFusion:
    ...
    def _update_rolling_stats(self, 
        ...
        if len(self.rolling_stats["price_mean"]) > 5:
            correlation = np.corrcoef(
                self.rolling_stats["price_mean"][-10:],
                self.rolling_stats["sentiment_mean"][-10:]
            )[0, 1]
    ...
    def _adapt_weights(self, regime: MarketRegime):
        ...
        price_weight = (1 - self.config.regularization_factor) * price_weight + \
                      self.config.regularization_factor * self.config.base_price_weight
        sentiment_weight = (1 - self.config.regularization_factor) * sentiment_weight + \
                          self.config.regularization_factor * self.config.base_sentiment_weight
    ...
    def _calculate_fusion(self, price_signal: float, sentiment_signal: float) -> float:
        ...
        return (self.current_weights["price"] * price_signal + 
