...
import requests
import feedparser
from src.data.crawler import PriceCrawler, NewsCrawler, DataCrawler
...
class TestDataCrawler:
    ...
    def test_crawl_news_rss_only(self, mock_newsapi, mock_rss):
        ...
        mock_rss.return_value = [
            {"title": "News 1", "ticker": "SPY"},
            {"title": "News 2", "ticker": "NVDA"}
        ]
    ...
    def test_crawl_news_both_sources(self, mock_newsapi, mock_rss):
        ...
        mock_rss.return_value = [
            {"title": "RSS News 1", "ticker": "SPY"}
        ]
        mock_newsapi.return_value = [
            {"title": "NewsAPI News 1", "ticker": "NVDA"}
        ]
    ...
    def test_crawl_news_deduplication(self, mock_newsapi, mock_rss):
        ...
        mock_rss.return_value = [
            {"title": "Same News", "ticker": "SPY"},
            {"title": "Unique News 1", "ticker": "NVDA"}
        ]
        mock_newsapi.return_value = [
