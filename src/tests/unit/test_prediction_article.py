...
class TestFinancialLSTM:
    ...
    def test_lstm_initialization(self):
        """Test initialisation modèle"""
        model = FinancialLSTM(input_size=4, hidden_size=64, num_layers=2, dropout=0.2)
        assert model.hidden_size == 64
        assert model.num_layers == 2

    def test_lstm_forward_pass(self):
        """Test forward pass"""
        model = FinancialLSTM(input_size=4, hidden_size=64, num_layers=2)
        model.eval()

        # Batch de 10 séquences de 216 timesteps, 4 features
        x = torch.randn(10, 216, 4)
