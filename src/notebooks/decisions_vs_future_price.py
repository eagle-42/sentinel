...
def create_plot(results, prices, window_days=14):
    ...
    for _, row in recent_results.iterrows():
        ...
        ax.scatter(
            row["timestamp"], row["close"], marker=marker, s=100, c=color, edgecolors="black", linewidth=1, alpha=0.8
        )

    # Configuration du graphique
    ax.set_title("Décisions vs Prix Futur - Validation des Prédictions", fontsize=16, fontweight="bold", pad=20)
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], marker="^", color="black", linestyle="None", markersize=8, label="BUY"),
        plt.Line2D([0], [0], marker="v", color="black", linestyle="None", markersize=8, label="SELL"),
        plt.Line2D([0], [0], marker="o", color="black", linestyle="None", markersize=8, label="HOLD"),
    ]
...
def main():
    ...
    parser.add_argument(
        "--decisions",
        default="data/trading/decisions_log/trading_decisions.json",
        help="Chemin vers le fichier de décisions",
    )
    parser.add_argument(
        "--prices", default="data/realtime/prices/spy_15min.parquet", help="Chemin vers le fichier de prix"
    )
    parser.add_argument(
        "--horizon-steps", type=int, default=1, help="Nombre de pas dans le futur pour calculer le rendement"
    )
    parser.add_argument("--window-days", type=int, default=14, help="Fenêtre de temps à afficher (jours, 0 pour tout)")
    ...
    parser.add_argument("--output-dir", default="src/notebooks", help="Répertoire de sortie")
    ...
    for _, kpi in kpis.iterrows():
        print(
            f"{kpi['decision']:>6}: {kpi['n']:>3} décisions | "
